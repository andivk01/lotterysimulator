...
public class Type1 extends AbstractPlayer {
	private static final long serialVersionUID = 1L;

	public Type1(ExtractionsData ed){
		this("unknown", "", ed, 10);
	}

	public Type1(String name, double profit, double gain, double loss, int quantityBets) {
		super(name, profit, gain, loss, quantityBets);
	}

	public Type1(String name, String surname, ExtractionsData ed, int storyOfPlayerEach) {
		super(name, surname, ed, storyOfPlayerEach);
		this.type = PlayerType.T1;
	}

	@Override
	public void play() {
		this.bet = ed.getRetarded(bet.length);
		if(storyOfPlayerEach<=0) storyOfPlayerEach = 10;
		if((this.quantityBets % storyOfPlayerEach) == 0) {
			story.add(new Type1(name, this.profit, this.gain, this.loss, this.quantityBets));
		}
	}
